#!/usr/bin/env python
"""
Test the complex model (TF-IDF + MLPClassifier) in isolation.
"""
import sys
import os
import time
...
def test_complex_model(dataset_name="imdb", sample_size=50, n_trials=2):
    """Test the complex model with TF-IDF + MLPClassifier."""
    print("\n" + "="*80)
    print(f"Testing COMPLEX model on {dataset_name} dataset")
    print("="*80)
    ...
    # Create config for complex model
    config = BOHBConfig(
        max_budget=3.0,
        min_budget=1.0,
        n_trials=4,  # Must be greater than max_budget
        wall_clock_limit=180.0,  # 3 minutes
    ...
    print(f"Starting complex model optimization...")
    ...
    try:
        ...
        print(f"\n✅ Complex model optimization completed in {elapsed_time:.2f}s")
        ...
        elapsed_time = time.time() - start_time
        print(f"\n❌ Complex model optimization failed: {e}")
        import traceback
        traceback.print_exc()
        return False, 0.0, {}

if __name__ == "__main__":
    # Run complex model test with default parameters
    success, score, config = test_complex_model()
