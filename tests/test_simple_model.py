#!/usr/bin/env python
"""
Test the simple model (TF-IDF + LogisticRegression/SVM) in isolation.
"""
import sys
import os
import time
...
def test_simple_model(dataset_name="imdb", sample_size=100, n_trials=2):
    """Test the simple model with TF-IDF + LogisticRegression/SVM."""
    print("\n" + "="*80)
    print(f"Testing SIMPLE model on {dataset_name} dataset")
    print("="*80)
    ...
    config = BOHBConfig(
        ...
        min_budget=1.0,
        n_trials=n_trials,
        wall_clock_limit=60.0,  # 1 minute
    ...
    print(f"Starting simple model optimization...")
    ...
    try:
        ...
        print(f"\n✅ Simple model optimization completed in {elapsed_time:.2f}s")
        ...
        elapsed_time = time.time() - start_time
        print(f"\n❌ Simple model optimization failed: {e}")
        import traceback
        traceback.print_exc()
        return False, 0.0, {}

if __name__ == "__main__":
    # Run simple model test with default parameters
    success, score, config = test_simple_model()
