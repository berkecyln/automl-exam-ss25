...
class AutoMLPipeline:
    ...
    def _log_progress(self, stage: str, message: str, data: Dict[str, Any] = None):
        ...
        progress_msg = (
            f"[{elapsed/60:.1f}m elapsed, {remaining/60:.1f}m remaining] {stage}: {message}"
        )
        print(progress_msg)
        ...
        for dataset_name in datasets:
            try:
                ...
                if raw_meta_features is None:
                    self._log_progress(
                        "ERROR",
                        f"Failed to extract features for {dataset_name}: raw_meta_features is None",
                    )
                    break
    ...
    def _run_iterative_rl_bohb_training(
        self, training_datasets: List[Tuple], prefix: str = ""
    ) -> RLModelSelector:
        """
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            for idx, (texts, labels, mfeat) in enumerate(training_datasets):
                ...
                bohb_cfg = self.make_bohb_profile(
                    iteration, max_iterations, self.bohb_base, model_type=model_type, is_test=False
                )
                bo = BOHBOptimizer(
                ...
                self.results["detailed_logs"].append(
                    {
                        ...
                        "bohb": {
                            ...
                            "incumbent_history": bohb_info.get("optimization_stats", {}).get(
                                "convergence_history", []
                            )[-10:],
            ...
            self._log_progress(
                f"{log_prefix}ITER_DONE", f"iter  {iteration}: meanacc {mean_acc:.4f}"
            )
    ...
    def run_leave_one_out_cv(self):
        ...
        """
        self.results["cv_results"] = {"folds": [], "performance": {}, "summary": {}}
        ...
        for held_out in self.datasets:
            ...
            if held_out not in self.results["meta_features"]:
                self._log_progress(
                    "CV_ERROR", f"Missing meta-features for {held_out}, skipping fold"
                )
                continue

            # Run RL+BOHB training on remaining datasets
            rl_selector = self._run_iterative_rl_bohb_training(
                training_datasets, prefix=f"cv_{held_out}"
            )
            ...
            self._log_progress(
                "CV_EVALUATION", f"Selected {model_type} for held-out dataset {held_out}"
            )
            # Run with max budget for final evaluation for leaved out dataset
            final_cfg = self.make_bohb_profile(
            ...
            if fanova_imp:
                self._log_progress("CV_FANOVA", f"{held_out}: top HP importances {fanova_imp}")
            else:
                self._log_progress("CV_FANOVA", "no fanova importances available")
            ...
            self._log_progress(
                "CV_RESULT", f"Held-out {held_out}: {model_type} model achieved {best_score:.4f}"
            )

            # Save CV fold results
            self._save_intermediate_results("cv", held_out)
    ...
    def _run_final_training(self, is_test: bool = True) -> Dict[str, Any]:
        ...
        top_k = 5
        self._log_progress("INTERP_META", f"Top-{top_k} meta-features influencing RL choice")
        ...
        if final_selection:
            all_scores = [
                sel["bohb_score"]
                for sel in final_selection.values()
                if sel["bohb_score"] is not None
            ]
    ...
    def _predict_on_test_split(self, dataset: str = "yelp", data_path: str = "data") -> np.ndarray:
        ...
        self._log_progress(
            "PREDICT:",
            f"Predicting on {dataset} using {model_type} from agent {best_agent} with config: {best_config}",
        )
        print(best_config)
        ...
        try:
            ...
            try:
                explainer = LimeTextExplainer(
                    class_names=[str(c) for c in sorted(set(labels_train))]
                )
                ...
                for idx in sample_idx:
                    exp = explainer.explain_instance(
                        texts_test[idx], model.predict_proba, num_features=5, labels=[preds[idx]]
                    )
                ...
                self._log_progress("LIME_DETAILS", f"Explanations: {explanations}")
                # keep them in results so you can inspect/save later
                self.results.setdefault("lime_explanations", []).extend(explanations)
    ...
    def _save_intermediate_results(self, stage: str, context: str = None):
        """Save intermediate results with optional context.

        Args:
            stage: Stage name (e.g., 'meta_features', 'training', 'final')
            context: Optional context (e.g., dataset name for CV)
        """
        try:
            # Create stage identifier
            stage_id = f"{stage}_{context}" if context else stage

            results_file = self.output_dir / f"intermediate_{stage_id}_results.pkl"
            ...
            with open(self.output_dir / f"intermediate_{stage_id}_summary.json", "w") as f:
