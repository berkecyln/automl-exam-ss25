...
class BaseTextDataset(ABC):
    ...
    def create_dataloaders(self, val_size: float = 0.2, random_state: int = 42) -> Dict[str, Any]:
    ...
    def load_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        if train_path.exists() and test_path.exists():
            ...
            raise FileNotFoundError(
                f"Data files not found at {train_path}, generating dummy data..."
            )
...
def load_dataset(
    dataset_name: str, split: str = "train", data_path: str = "data"
) -> Tuple[list, list]:
    ...
    if dataset_name not in dataset_classes:
        raise ValueError(
            f"Unknown dataset: {dataset_name}. Available: {list(dataset_classes.keys())}"
        )
