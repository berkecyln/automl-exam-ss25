...
class AutoMLLogger:
    ...
    def log_stage_end(self, stage_name: str, results: Dict[str, Any] = None, elapsed_time: float = None):
    ...
    def log_dataset_info(self, dataset_name: str, dataset_info: Dict[str, Any]):
        ...
        self.pipeline_state['dataset_info'] = {
            'name': dataset_name,
            **dataset_info
        }
    ...
    def log_bohb_iteration(
        ...
        simple_params = {k: (round(v, 4) if isinstance(v, float) else v) 
                        for k, v in hyperparams.items() if k in ['lr', 'batch_size', 'epochs', 'hidden_dim']}
    ...
    def log_final_results(
        ...
        # Simplify hyperparams for logging
        simple_params = {k: (round(v, 4) if isinstance(v, float) else v) 
                        for k, v in best_hyperparams.items() if k in ['lr', 'batch_size', 'epochs', 'hidden_dim']}
