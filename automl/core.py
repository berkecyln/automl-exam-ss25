...
class TextAutoML:
    ...
    def fit(
        ...
        if weight_decay is not None:
            self.weight_decay = weight_decay
        if ffnn_hidden is not None:
            self.ffnn_hidden = ffnn_hidden
        if lstm_emb_dim is not None:
            self.lstm_emb_dim = lstm_emb_dim
        if lstm_hidden_dim is not None:
            self.lstm_hidden_dim = lstm_hidden_dim
        if fraction_layers_to_finetune is not None:
            self.fraction_layers_to_finetune = fraction_layers_to_finetune
        ...
        if self.approach == "tfidf":
            ...
            dataset = torch.utils.data.TensorDataset(
                torch.tensor(X, dtype=torch.float32), torch.tensor(self.train_labels)
            )
            ...
            _dataset = torch.utils.data.TensorDataset(
                torch.tensor(X, dtype=torch.float32), torch.tensor(self.val_labels)
            )
    ...
    def _train_loop(
        ...
        if load_path is not None:
            _states = torch.load(
                load_path / "checkpoint.pth", map_location="cpu"
            )  # Load to CPU first
    ...
    def predict(self, test_data: pd.DataFrame | DataLoader) -> Tuple[np.ndarray, np.ndarray]:
        ...
        assert isinstance(test_data, DataLoader) or isinstance(
        ...
        if self.approach == "tfidf":
            ...
            _dataset = torch.utils.data.TensorDataset(
                torch.tensor(_X, dtype=torch.float32), torch.tensor(_labels)
            )
