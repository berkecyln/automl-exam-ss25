    ...
    def fit(self, X_train: List[str], y_train: List[int]) -> "SimpleModel":
        ...
        if self.config["algorithm"] == "logistic":
            self.model = LogisticRegression(
                C=self.config["C"], max_iter=self.config["max_iter"], random_state=self.random_state
            )
        elif self.config["algorithm"] == "svm":
            self.model = SVC(C=self.config["C"], kernel="linear", random_state=self.random_state)
...
class MediumModel(BaseTextModel):
    ...
    def fit(self, X_train: List[str], y_train: List[int]) -> "MediumModel":
        ...
        svd = TruncatedSVD(n_components=svd_components, random_state=self.random_state)
    ...
    def predict_proba(self, X_test: List[str]) -> np.ndarray:  # NEW
        ...
        X_test_svd = self.preprocessor.transform(self.vectorizer.transform(X_test))
...
class ComplexModel(BaseTextModel):
    ...
    def __init__(
        self, config: Dict[str, Any], random_state: int = 42, budget_fraction: float = 1.0
    ):
        super().__init__(random_state)
        self.config = config
...
def create_model(
    model_type: str, config: Dict[str, Any], random_state: int = 42, **kwargs
) -> BaseTextModel:
