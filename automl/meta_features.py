...
class MetaFeatureExtractor:
    ...
    def extract_features(
        self, train_df: pd.DataFrame, val_df: pd.DataFrame = None, num_classes: int = None
    ) -> Dict[str, float]:
    ...
    def _extract_basic_stats(
        self, texts: list, labels: list = None, num_classes: int = None
    ) -> Dict[str, float]:
            ...
            vocab_stats.update(
                {
                    ...
                    "hapax_legomena_ratio": sum(1 for count in word_counts.values() if count == 1)
                    / len(vocab),
                    "top_10_word_freq_ratio": sum(sorted(word_counts.values(), reverse=True)[:10])
                    / len(all_words),
                }
            )
    ...
    def _extract_baseline_performance(self, texts: list, labels: list) -> Dict[str, float]:
        ...
        try:
            ...
            cv_scores = cross_val_score(
                pipeline, texts, labels, cv=5, scoring="accuracy", n_jobs=-1
            )
    ...
    def get_ordered_features(
        self, meta_features: Dict[str, float], normalize: bool = True
    ) -> Dict[str, float]:
